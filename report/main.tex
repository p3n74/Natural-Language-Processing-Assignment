...
\author{Nikolai Tristan Pazon\\Heart Alvern Sumicad\\Jade De Paz\\Rameses Luis C. Barria}
\date{\today}
...
\section{Pipeline Timeline}
\label{sec:timeline}

We kept a lab notebook (\path{notebooks/02_pipeline_timeline.ipynb}) that documents the questions, hypotheses, and decisions behind each stage of the pipeline. Table~\ref{tab:timeline} condenses that timeline into a single view so the workflow is easy to audit.

\begin{table}[ht]
  \centering
  \footnotesize
  \caption{Pipeline timeline distilled from the lab notebook.}
  \label{tab:timeline}
  \begin{tabularx}{\linewidth}{@{}clX@{}}
    ...
    7 & Training & 5-fold CV over $C/\alpha$ and n-gram range. \\
...
\textbf{Reproducibility.} All processed CSVs, trained models, and figures can be regenerated with \texttt{make data \&\& make train \&\& make evaluate \&\& make errors \&\& make report} from the repository root.
...
\begin{table}[ht]
  \centering
  \footnotesize
  \caption{Team contributions for the Letterboxd sentiment project.}
  \label{tab:contrib}
  \begin{tabularx}{\linewidth}{@{}r l X r@{}}
    \toprule
